...
class mesh_crowd(object):
    def __init__(self, curr_archiving_in, curr_archiving_fit, mesh_div, particals):
        self.curr_archiving_in = curr_archiving_in  # 当前存档中所有粒子的坐标
        self.curr_archiving_fit = curr_archiving_fit  # 当前存档中所有粒子的适应度
        ...
        self.id_archiving = np.zeros((self.num_))  # Archive集中各个粒子的id编号，检索位与curr_archiving的检索位为相对应
        ...
        self.f_max = np.max(np.array(self.curr_archiving_fit), axis=0)  # 求网格的取值范围
        self.f_min = np.min(np.array(self.curr_archiving_fit), axis=0)
        self.fd = self.f_max - self.f_min


    def cal_mesh_id(self, fit_):
        # 计算网格编号id
        ...
        for i in range(self.curr_archiving_fit.shape[1]):
            id_dim = int((fit_[i] - self.f_min[i]) * self.num_ / (self.fd[i])) 
...
class get_gbest(mesh_crowd):
    def __init__(self, curr_archiving_in, curr_archiving_fit, mesh_div, particals):
        super(get_gbest, self).__init__(curr_archiving_in, curr_archiving_fit, mesh_div, particals)
...
class clear_archiving(mesh_crowd):
    def __init__(self, curr_archiving_in, curr_archiving_fit, mesh_div, particals):
        super(clear_archiving, self).__init__(curr_archiving_in, curr_archiving_fit, mesh_div, particals)
        self.divide_archiving()
        self.get_crowd()
    ...
    def clear_(self, thresh):
        ...
        self.curr_archiving_in = np.delete(self.curr_archiving_in, clear_index, axis=0)  # 初始化gbest矩阵_坐标
        self.curr_archiving_fit = np.delete(self.curr_archiving_fit, clear_index, axis=0)  # 初始化gbest矩阵_适应值
