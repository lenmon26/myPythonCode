...
def compare_(fitness_curr, fitness_ref):
    # 判断fitness_curr是否可以被fitness_ref完全支配//如果是，则返回True
    for i in range(len(fitness_curr)):  # 每个粒子适应度有3个
...
def judge_(fitness_curr, fitness_data, cursor):
    ...
    for i in range(len(fitness_data)):
        if i == cursor:
            continue
        # 如果数据集中存在一个粒子可以完全支配当前解，则证明当前解为劣解，返回False//应改为True
        if compare_(fitness_curr, fitness_data[i]) == True:
            return True
    return False   
...
class Pareto_:
    def __init__(self, in_data, fitness_data):
        ...
        self.len_ = in_data.shape[0]  # 粒子群的数量 /（100）
    ...
    def pareto(self):
        while (self.hasNext()):
            ...
            # 判断当前粒子是否pareto最优
            if judge_(fitness_curr, self.fitness_data, self.cursor) == True:  # fitness_curr被self.fitness_data支配
                self.remove()
